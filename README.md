...
### 🪟 Шторы, вентиляторы и замки

Кроме света, розеток, термостатов и любых сенсоров поддерживаются:

- **`cover`** — шторы, рольставни, ворота: положение 0–100 и команды
  открыть/стоп/закрыть, `device_class` (`curtain`, `blind`, `shutter`,
  `garage`, `gate`…);
- **`fan`** — вентиляторы: вкл/выкл, скорость (пресеты или проценты),
  направление (`forward` / `reverse`);
- **`lock`** — умные замки: `LOCK` / `UNLOCK`, поле `inverted` меняет смысл
  значения DP.

Как описать их в конфиге — в разделе «Конфиг устройств».
...
| `SWITCH_DEBOUNCE_MS` | `0` | Дебаунс (склейка) быстрых команд `switch`/`light`: **первая** уходит сразу, хвост серии — одной финальной. `0` = выключено, `600` = включить защиту от спама кликов |
| `SWITCH_DEBOUNCE_MAX_MS` | `1200` | Потолок ожидания финальной команды при спаме, мс (ограничивает «залипон») |
...
| `type` | Тип устройства: `light`, `switch`, `climate`, `sensor`, `binary_sensor`, `cover`, `fan` |
...
**Шторы, вентиляторы, замки** (коды DP берутся из вашего устройства — имена фиксированы):

- `type: "cover"` — DP `control` (`open` / `stop` / `close`),
  `percent_control` (целевое положение 0–100), `percent_state` (текущее).
  У DP `control` можно указать `device_class` (`curtain`, `garage`, `shutter`…);
- `type: "fan"` — DP `switch` (вкл/выкл), `fan_speed` (со списком `options` →
  пресеты, без него число → проценты), `fan_direction` (`forward` / `reverse`);
- замок — DP с `component: "lock"` (bool, обычно `lock_state`);
  поле `inverted: true` меняет смысл значения.
